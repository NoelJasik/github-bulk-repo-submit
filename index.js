...
const dir = 'D:/Projects/Unity/Experiments/Done';
...
fs.readdir(dir, (err, files) => {
    ...
    // Create a repository for each subdirectory that doesn't contain a .git folder
    files.forEach((file) => {
        if (!file.endsWith(".zip")) {
            const repoName = file;

            // Check if the subdirectory contains a .git folder
            const gitDir = `${dir}/${repoName}/.git`;
            fs.stat(gitDir, (err, stats) => {
                // If the .git folder doesn't exist
                if (err) {
                                    ...
                                    exec(`git add .`, { cwd: localDir }, (err, stdout, stderr) => {
                                        ...
                                        exec(`git commit -m "Initial commit"`, { cwd: localDir }, (err, stdout, stderr) => {
                                            ...
                                            exec(`git push -u origin master`, { cwd: localDir }, (err, stdout, stderr) => {
                                                ...
                                                console.log(`Successfully pushed local directory to repository: ${body.html_url}`);
                                            });
                                        });
                                    });
                                });
                            ...
                            exec(`git init`, { cwd: localDir }, (err, stdout, stderr) => {
                                ...
                                exec(`git remote add origin ${cloneUrl}`, { cwd: localDir }, (err, stdout, stderr) => {
                                    ...
                                    exec(`git add .`, { cwd: localDir }, (err, stdout, stderr) => {
                                        ...
                                        exec(`git commit -m "Autoupdate!"`, { cwd: localDir }, (err, stdout, stderr) => {
                                            if (err) {
                                                console.error(`Error pushing to repository: ${err}`);
                                                return;
                                            }
